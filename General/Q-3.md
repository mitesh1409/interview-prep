...
So Customer extends from BaseUser and system users like Admin, SuperAdmin, CustomerSupport and TechSupport extends from SystemUser.  

Class hierarchy is as follows:  
`Customer` $\rightarrow$ `BaseUser`  

`Admin`, `SuperAdmin`, `CustomerSupport`, `TechSupport` $\rightarrow$ `SystemUser` $\rightarrow$ `BaseUser`  
