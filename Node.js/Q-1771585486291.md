...
2. Then we found a problem with "Nested Callbacks" - "Callback Pyramid of Doom"/"Callback Hell".  
...
## 1. Using Callbacks — Problem with Nested Callbacks -> Callback Pyramid of Doom or Callback Hell
...
```javascript
// 1 - read the file, output - raw file data
// 2 - process data, output - processed data
// 3 - prepare reports, output - reports

readFile('data.txt', function(err, rawData) {
    if (err) {
        console.error('Error reading file:', err);
        return;
    }
    processData(rawData, function(err, processedData) {
        if (err) {
            console.error('Error processing data:', err);
            return;
        }
        prepareReports(processedData, function(err, reports) {
            if (err) {
                console.error('Error preparing reports:', err);
                return;
            }
...
## 3. Using async/await — Looks Like Synchronous Code
...
|---|---|---|---|
| Shape | Nested callbacks endup in pyramid | Flat chain of `then()`, `catch()` & `finally()` | Linear/sequential |
| Error handling | Per callback & repeated | Single `catch()` | Single `try/catch` |
| Readability | Poor at scale | Good | Best |
| Under the hood | Raw callbacks | Promise API | Promise API |

`async/await` didn't replace Promises — it's built on top of them. Knowing how Promises work is essential to understanding `async/await` behavior.
