# Explain `process.nextTick()` in Node.js OR What is the difference between `process.nextTick()`, `setImmediate()` and `setTimeout()`?
...
> “Run this **right after the current code**, don’t wait for Microtasks Queue or Event Loop Phases.”
...
> “Finish what you’re doing now, then run this callback **ASAP**.”
...
* It waits until **current call stack finishes**
* Then it runs **before** the event loop moves on

---

## How `process.nextTick()` Works (Internals – Simple)
...
👉 That double-check is `process.nextTick()`

You don’t stop the current work,
you just say:

> “As soon as I’m done, do this first.”
...
✔ Ensures listeners are registered  
✔ Very common Node.js pattern  
...
### Solution using `process.nextTick()` ✅
...
## ⚠️ Important Warning (Very Important for Interviews)
...
> ❌ Don’t use `process.nextTick()` in recursive or heavy loops
...
| ------- | -----------------| ------------ |
| Runs | Before Microtasks Queue & Event Loop Phases | In the Check phase of the Event Loop |
| Priority | Highest | Lower |
| Risk | Can starve Microtasks Queue & Event Loop | Safer |
...
* You must run **before promises and the Event Loop**
...
**Tick means when current call stack becomes empty and then Microtasks Queue and then Event Loop continues**  
Yes, this is essentially correct. A "tick" is the moment the current call stack drains and just before the control returns to Microtasks Queue and then Event Loop. `process.nextTick()` hooks into *that* moment — its callbacks run right then, before Microtasks Queue and then Event Loop.

**The correct mental model**  
`nextTick` callbacks run **at the end of the current operation, before Microtasks Queue and then Event Loop**.
...
This is actually why `process.nextTick()` can be dangerous — if you recursively schedule `nextTick` callbacks inside `nextTick` callbacks, you can starve the event loop indefinitely, because Node.js will keep draining the `nextTick` queue before ever moving on.
