...
* Asynchronous operations are handed over to libuv and callbacks are **registered**, not executed
...
> Asynchronous code is NOT executed here — asynchronous operations are handed over to libuv and corresponding callbacks are registered.
...
👉 **Now control is transferred to libuv -> event loop**
...
| Event loop           | libuv                              |
| Async I/O            | libuv via thread pool or OS kernel |
...
2. Pending Callbacks → Some system-level callbacks (e.g., TCP errors)
3. Idle, Prepare     → Internal use (Node.js internals)
4. Poll              → I/O callbacks (fs, net, http)
5. Check             → setImmediate
6. Close Callbacks   → socket.on('close'), cleanup callbacks
```
...
* Promises run **after nextTick, before the Event Loop proceeds**
* `setTimeout(0)` runs in **Timers phase**
* I/O runs in **Poll phase**
* `setImmediate` runs in **Check phase**
* nextTick Queue & Microtasks Queue are completely drained everytime after call stack becomes empty and before the event loop phases
