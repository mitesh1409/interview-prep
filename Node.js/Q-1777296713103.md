...
You should use worker threads in the following scenarios:  
...
In this example, the main thread creates a worker thread that performs  
a CPU-intensive task (calculating the sum of numbers from 0 to 1 billion)  
without blocking the main event loop.  
The result is sent back to the main thread using `parentPort.postMessage()`.
