...
* Server generates a JWT and signs it with a secret key, and then sends it to the client.
  This token contains all the required data set by server.

* Client can read the token but cannot modify it without a valid secret key.
  Only server has this secret key.
...
* Server generates a token in a way that -
  ...
  * encrypts/signs it using a secret key, tokens are signed
  * client can read the token but cannot modify it without a valid secret key (only server has this secret key)
* Server can send this token to the client using any one of the following ways:
  * cookie (for web applications)
  * directly in the response (for native apps)
...
In case of Serverless Architecture, stateful authentication can be implemented  
by saving session data into the database (persistent storage).  

Where application tier and database tier are different.
