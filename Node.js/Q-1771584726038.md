...
* **The Single Rule:** If the Call Stack is empty (meaning the current synchronous code block has finished executing), the Event Loop takes the first callback from the Event Queue and pushes it onto the Call Stack for execution (Priority Order ---> Next tick queue > Microtask/Priority Queue > Task/Callback Queue).
...
Here are the phases of the Event Loop:
