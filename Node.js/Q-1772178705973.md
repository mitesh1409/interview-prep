...
Without streams, your RAM usage spikes to the size of the file.  
With streams, Node.js processes it in chunks (default **64KB** each), keeping memory usage consistently low regardless of file size.
...
readStream              // Read: read a file
    .pipe(gzip)         // Transform: compress data
    .pipe(writeStream); // Write: Write compressed data to a file
...
|---|---|---|---|
| Readable | Read only | `fs.createReadStream()`, `http.IncomingMessage` (incoming HTTP request), `process.stdin` | Reading files, HTTP request body |
| Writable | Write only | `fs.createWriteStream()`, `http.ServerResponse` (outgoing HTTP response), `process.stdout` | Writing files, HTTP response |
| Duplex | Read + Write (independent) | `net.Socket`, `crypto.createCipheriv()`, WebSocket connections | TCP connections, WebSockets |
| Transform | Read (from source) + Transform (modify) + Write (to destination) | `zlib.createGzip()`, `zlib.createGunzip()`, `crypto.createCipheriv()`, custom data transformation pipelines | Compression, encryption, parsing |
