...
2. **Web APIs (or Node.js APIs):**
    ...
    * When an asynchronous function is called, it's passed to the relevant Web API or Node.js API for processing in the background.
...
1. **Run Sync Code:**  
The JavaScript engine executes all synchronous code. Function calls are pushed onto and popped off the **Call Stack**.
2. **Delegate Async:**  
When an asynchronous function (like `setTimeout`) is encountered,  
first it is pushed into the call stack but since it is asynchronous it is then popped off and handed over to Web API or Node.js API,  
JavaScript Engine moves to the next synchronous instruction,  
Web API or Node.js API runs that asynchronous function in the background without blocking the main thread,  
once that asychronous function finishes its execution its callback is then pushed into the relevant Callback Queue.
3. **Wait in Queue:**  
Callbacks of the asynchronous functions wait in their relevant queues until they are picked up by Event Loop for execution.
4. **The Loop Check (The Magic):**  
The Event Loop constantly checks:  
    * **Is the Call Stack Empty?** (No synchronous code is running).
    * **Is there a waiting callback in the Task Queue?**
5. **Push to Stack:**  
If the Call Stack is empty, the Event Loop takes the first callback from the Task Queue and **pushes it onto the Call Stack** for execution. This cycle ensures that asynchronous code only runs *after* all synchronous code has finished.
