...
Lexical Scope = Current scope + immediate parent/outer scope  
This is defined at the time of writing code, it is static and fixed.
...
`let`/`const`: Lexically scoped to the Block `{...}`.  
`var`: Lexically scoped to the Function `function() {...}`.

The **Rule** for where a variable/function can be accessed.

**Lexical Environment (Container)**  
This is created while executing code as per the Lexical Scope.

The **Container** that holds the actual variables, functions and references for a scope.

**Scope Chain (Path)**  
This is the hierarchy of lexical scopes that the JavaScript engine uses to look up variables.

The **Path** the engine follows to find variables, functions according to the lexical scope.

## The Scope Chain (The Lookup Mechanism)

The **Scope Chain** is the hierarchy of lexical scopes that the JavaScript engine uses to look up for variables/functions.
...
function outerFunction() {
    ...
    function innerFunction() {
        ...
        // Lookup `something`: Not found anywhere in the scope chain
        // Throws ReferenceError: something is not defined
...
// unless running in non-strict mode where it might implicitly create a global variable

strict mode example

non srict mode example
